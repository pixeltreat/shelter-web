...
define(['Boiler', './viewmodel', 'text!./view.html', 'i18n!./nls/resources'], function (Boiler, ViewModel, template, nls) {
    ...
    var Component = function (moduleContext) {
        ...
        moduleContext.listen($ct.en.getRefreshView(), function (viewName) {
            if ($ct.rn.getStaffTypeList() == viewName) {
                refreshViewData();
            }
        });
        ...
        moduleContext.listen($ct.en.getStaffTypeCreatedOrUpdated(), function () {

            if (vm) {
                vm.data.refreshStaffTypeGrid();
            }
        });


        moduleContext.listen($ct.en.getDisplayStaffTypeList(), function () {
        ...
        this.activate = function (parent, params) {
            ...
            moduleContext.parentContext.currentView = $ct.rn.getStaffTypeList();
            ...
            if (!panel) {
                ...
                $("#vwStaffTypeList, #vwstlDgStafftype").delegate("tbody>tr", "dblclick", function (element) {
                    Boiler.UrlController.goTo(vm.data.staffTypetEditUrl(false));
                });
