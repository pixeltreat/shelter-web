...
define(['Boiler', './viewmodel', 'text!./view.html', 'i18n!./nls/resources'], function (Boiler, ViewModel, template, nls) {
    ...
    var Component = function (moduleContext) {
        ...
        moduleContext.listen($ct.en.getRefreshView(), function (viewName) {
            if ($ct.rn.getQuestionList() == viewName) {
                refreshViewData();
            }
        });
        ...
        moduleContext.listen($ct.en.getQuestionCreatedOrUpdated(), function () {

            if (vm) {
                vm.data.refreshdsQuestionGrid();
            }
        });


        moduleContext.listen($ct.en.getDisplayquestionList(), function () {
        ...
        this.activate = function (parent, params) {
            ...
            moduleContext.parentContext.currentView = $ct.rn.getQuestionList();
            ...
            if (!panel) {
                ...
                $("#vwQuestionList, #vwqlDgQuestion").delegate("tbody>tr", "dblclick", function (element) {
                    Boiler.UrlController.goTo(vm.data.questionEditUrl(false));
                });
