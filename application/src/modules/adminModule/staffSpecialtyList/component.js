...
define(['Boiler', './viewmodel', 'text!./view.html', 'i18n!./nls/resources'], function (Boiler, ViewModel, template, nls) {
    ...
    var Component = function (moduleContext) {
        ...
        moduleContext.listen($ct.en.getRefreshView(), function (viewName) {
            if ($ct.rn.getStaffSpecialtyList() == viewName) {
                refreshViewData();
            }
        });
        ...
        moduleContext.listen($ct.en.getStaffSpecialtyCreatedOrUpdated(), function () {

            if (vm) {
                vm.data.refreshStaffSpecialtyGrid();
            }
        });


        moduleContext.listen($ct.en.getDisplayStaffSpecialtyList(), function () {
        ...
        this.activate = function (parent, params) {
            ...
            moduleContext.parentContext.currentView = $ct.rn.getStaffSpecialtyList();
            ...
            if (!panel) {
                ...
                $("#vwStaffSpecialtyList, #vwsslDgStaffSpecialty").delegate("tbody>tr", "dblclick", function (element) {
                    Boiler.UrlController.goTo(vm.data.staffSpecialtyEditUrl(false));
                });
